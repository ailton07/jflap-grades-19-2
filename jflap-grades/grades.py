...
def checkRunQuestion(test, fileName, studentName):
    ...
    for case in test['cases']:
        ...
        resultBin = -1
        if result != '':
            resultBin = (1 if 'true' in str(result) else 0)
...
def calculateExercises(exerciseFiles, studentName, testCase):
    ...
    for fileName in exerciseFiles:
        underscore_counter = fileName.count('_')
        if (underscore_counter == 2) :
            question = fileName[-8:-4].lower()
            ...
            try:
                ...
                print('Error in testCase[2][question]')
        else:
            question = fileName[-9:-4].lower()
            question_splited=question.split('_')
            if(len(question_splited) > 0):
                question = question_splited[len(question_splited)-2] + question_splited[len(question_splited)-1]
            print('--> [' + question + ']')
            try:
                test = testCase[2][question]
                print(test)
